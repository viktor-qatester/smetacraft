...
- `index.html`, `js/core/*.js`, `js/ui/slab.js` и `js/ui/strip.js`: Vanilla HTML/CSS/JS без сборки, backend, БД и LLM для расчётов. Параметры слева, закреплённая ведомость справа. Анкета «Мой дом», прайс-лист, печать, JSON import/export и `localStorage` уже существуют.
- `read*Form` читает DOM, `validate*` проверяет ввод; `calculateSlab`, `calculateStrip`, `calculateWalls`, `calculatePlaster`, `calculateRoof`, `calculateFloor` находятся в `js/core/*.js` с явными зависимостями от действующих констант и чистых помощников. `renderSummary` агрегирует выбранные части в `index.html`. Потоки плиты и ленты выделены в `js/ui/slab.js` и `js/ui/strip.js` в Phase 5A–5B; таблица свай и остальные UI-потоки ещё в `index.html`.
...
- `tests/scenarios.cjs` задаёт входы. `tests/runtime.cjs` исполняет рабочие функции из `index.html`, `js/ui/slab.js`, `js/ui/strip.js` и файлов ядра в Node VM в порядке загрузки страницей. `tests/golden.json` фиксирует строки, количества, цены, суммы и вывод; его `sourceSha256` — происхождение, не условие автоматического переписывания.
