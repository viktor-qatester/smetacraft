...
Снимок расчётной архитектуры после Phase 4F. Последовательность и проверенный статус фаз теперь хранятся в `docs/ROADMAP.md`; перед каждой фазой сверяй эту карту с актуальным кодом, тестами и заданием владельца. Старый `docs/AGENT_TRANSFER_BRIEF.md` ссылается на отсутствующие HANDOFF/NEXT_SESSION_PROMPT и не определяет текущий порядок фаз.

## Архитектура и смета

- `index.html` и `js/core/*.js`: Vanilla HTML/CSS/JS без сборки, backend, БД и LLM для расчётов. Параметры слева, закреплённая ведомость справа. Анкета «Мой дом», прайс-лист, печать, JSON import/export и `localStorage` уже существуют.
- `read*Form` читает DOM, `validate*` проверяет ввод; `calculateSlab`, `calculateStrip`, `calculateWalls`, `calculatePlaster`, `calculateRoof`, `calculateFloor` находятся в `js/core/*.js` с явными зависимостями от действующих констант и чистых помощников. `renderSummary` агрегирует выбранные части в `index.html`. UI-модули относятся к Phase 5.
...
- `tests/scenarios.cjs` задаёт входы. `tests/runtime.cjs` исполняет рабочие функции из `index.html` и файлов ядра в Node VM в порядке загрузки страницей. `tests/golden.json` фиксирует строки, количества, цены, суммы и вывод; его `sourceSha256` — происхождение, не условие автоматического переписывания.
