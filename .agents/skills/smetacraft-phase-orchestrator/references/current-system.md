...
Карта архитектуры: расчётный и frontend-контракты после Phase 5, локальный серверный сценарий Phase 6. Последовательность и проверенный статус фаз теперь хранятся в `docs/ROADMAP.md`; перед каждой фазой сверяй эту карту с актуальным кодом, тестами и заданием владельца. Старый `docs/AGENT_TRANSFER_BRIEF.md` ссылается на отсутствующие HANDOFF/NEXT_SESSION_PROMPT и не определяет текущий порядок фаз.

## Архитектура и смета

- `index.html`, `js/core/*.js`, `js/ui/*.js` и `js/app/*.js`: Vanilla HTML/CSS/JS без сборки, БД и LLM для расчётов. Параметры слева, закреплённая ведомость справа. Анкета «Мой дом», прайс-лист, печать, JSON import/export и `localStorage` уже существуют. Локальный `server/server.cjs` обслуживает явную проверку приёма проекта без хранения; расчёты остаются в браузере.
