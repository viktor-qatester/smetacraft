...
- `index.html`, `js/core/*.js`, `js/ui/*.js` и `js/app/*.js`: Vanilla HTML/CSS/JS без сборки, БД и LLM для расчётов. Параметры слева, закреплённая ведомость справа. Анкета «Мой дом», прайс-лист, печать, JSON import/export и `localStorage` уже существуют. Локальный `server/server.cjs`: static + `POST /api/project-check` (Phase 6) + SQLite migrate/readback (Phase 7) + файловое хранилище и explicit-text import PDF/DOCX (Phase 8, `docs/IMPORT_PDF.md`, факты с evidence в `js/app/extracted-facts.js`). API: loopback или `SMETACRAFT_PUBLIC_ORIGIN`; bind по умолчанию `127.0.0.1`, публично только при `SMETACRAFT_BIND=0.0.0.0`. GitHub Pages не прод API. Testers: Fornex, [`docs/DEPLOY_FORNEX.md`](../../../../docs/DEPLOY_FORNEX.md). Расчёты остаются в браузере.
